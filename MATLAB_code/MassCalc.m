...
%--------------------------------------------------------------------------
    ...
    k=0; %YES, *of course* you can use same method with Z=[1 1 ...1] for calculating Trophic levels T1, and it's prob cleaner, but both methods work. 
    while k<2%We need to iterate the while loop one extra time than min req'd to calculate all Masses - because you only just got the right mass for all the prey species of an apex predator
        ...
        k=k+1-isnan(sum(Mass1));%We need to do one additional loop after finding all the Masses, because the ones that we found last need to update each other.
    end
    Mass1
