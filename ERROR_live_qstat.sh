...
#$ -l h_rt=0:10:0
#$ -l h_vmem=1G
( qstat | grep  $JobID\$simnum_0 | cut -d' ' -f1 )
ls \$job_name.* | cut -d'.' -f3
xkcd=$( ls \$job_name.* | cut -d'.' -f3 | cut -d'o' -f2)
qstat -j $(ls \$job_name.* )
echo INBETWEEN
qstat -j $(ls \$job_name.* | cut -d'.' -f3 | cut -d'o' -f2)
y=5
z=$JobID\$simnum_0
echo HI THERE
echo \$xkcd \$y \$z
...
echo $xkcd $y $z
echo BYE
( qstat -j \$xkcd | grep maxvmem )
( qstat -j \$( qstat | grep  $JobID\$simnum_0 | cut -d' ' -f1 ) | grep maxvmem )
( ( qstat -j \$( qstat | grep  $JobID\$simnum_0 | cut -d' ' -f1 ) | grep maxvmem ) | cut -d'=' -f6 )
echo hi hello
