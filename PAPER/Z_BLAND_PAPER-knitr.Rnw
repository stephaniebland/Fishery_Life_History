...
\subsection*{Derivation of Von-Bert Equation used in my model}
...
\end{equation}
Now we can substitute length for weight using the equation ():
\begin{equation}
  W=qL^3
\end{equation}
where W is individual fish weight, q= is the conversion factor for length to weight, and L is individual fish length. We assume that this equation holds true across species and lifestages. We solve for length:
\begin{equation}
...
\par This equation gives you a Von-Bert growth curve in weight, as shown [Insert figure ref]. Using this curve we can solve for the weight at each lifestage. This is where we run into a problem, since there are several methods of doing this:

We now need to assign a minimum weight for each fish species so that we know when to stop calculating expected weights.
