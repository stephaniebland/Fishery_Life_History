...
%--------------------------------------------------------------------------
    ...
    while webindex<tot_nwebs 
        ...
        if length(surv_sp)>=nichewebsize*pers_thresh && isConnected(newnicheweb)==1 && length(fish_sp)>=3
            ...
            B0=B0(surv_sp);%biomass of all remaining species inbetween cycles (so after last one and before the one that's about to start) 
            B0mean=mean(x(end-meanlast:end,surv_sp));%mean biomasses of all extant species throughout last cycle
            B0std=std(x(end-meanlast:end,surv_sp));%standard deviations
            K=K(surv_sp);%carrying capacity
